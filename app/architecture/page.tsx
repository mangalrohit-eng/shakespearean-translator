...
export default function ArchitecturePage() {
  ...
  const connections = [
    { from: 'start', to: 'orchestrator', label: 'Excel File', step: 0 },
    { from: 'orchestrator', to: 'excelReader', label: 'Read Request', step: 1 },
    { from: 'excelReader', to: 'orchestrator', label: 'Raw Data', step: 1 },
    { from: 'orchestrator', to: 'parser', label: 'Parse Task', step: 2 },
    { from: 'parser', to: 'orchestrator', label: 'Structured Data', step: 2 },
    { from: 'orchestrator', to: 'analyzer', label: 'Analyze Task', step: 3 },
    { from: 'analyzer', to: 'orchestrator', label: 'Tagged Results', step: 3 },
    { from: 'orchestrator', to: 'emailComposer', label: 'Email Task', step: 4 },
    { from: 'emailComposer', to: 'orchestrator', label: 'Generated Emails', step: 4 },
    { from: 'orchestrator', to: 'complete', label: 'Final Output', step: 5 },
  ]
  ...
  const getDataPacketPosition = (from: keyof typeof agents, to: keyof typeof agents, progress: number) => {
    const fromAgent = agents[from]
    const toAgent = agents[to]
              ...
              {connections.map((conn, idx) => {
                ...
                return (
                  <g key={idx}>
                    ...
                    {isActive && (
                      <g className="data-packet-animated">
                        {[0.2, 0.5, 0.8].map((progress, i) => {
                          const pos = getDataPacketPosition(conn.from as keyof typeof agents, conn.to as keyof typeof agents, progress)
                          return (
                            <circle
                              key={i}
                              cx={pos.x}
                    ...
                    <text
                      ...
                      fill={isActive ? '#a100ff' : 'rgba(255, 255, 255, 0.4)'}
                      fontSize="11"
